...
<body>
    ...
    <div class="container-fluid mt-4">
        ...
        <% if ("estado-actualizado".equals(request.getParameter("success"))) { %>
            <div class="alert alert-success alert-dismissible fade show" role="alert">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover">
                            ...
                            <tbody>
                                ...
                                %>
                                    <tr <% if (esPendiente) { %>class="table-warning"<% } %>>
                                        <td>
                                        ...
                                        <td>
                                            <div class="btn-group btn-group-sm">
                                                <button class="btn btn-outline-primary" 
                                                        onclick="verDetalleContacto('<%= contacto.getNombre() %>', '<%= contacto.getEmail() %>', '<%= contacto.getAsunto() %>', '<%= contacto.getMensaje() %>', '<%= contacto.getFechaEnvio().format(formatter) %>')" 
                                                        title="Ver detalle">
                                                    <i class="bi bi-eye"></i>
                                                </button>
                                                <div class="btn-group btn-group-sm">
                                                    ...
                                                    <ul class="dropdown-menu">
                                                        <li>
                                                            <form method="post" action="<%= request.getContextPath() %>/admin/marcar-contacto/<%= contacto.getIdContacto() %>" style="display: inline;">
                                                        ...
                                                        <li>
                                                            <form method="post" action="<%= request.getContextPath() %>/admin/marcar-contacto/<%= contacto.getIdContacto() %>" style="display: inline;">
                                                                <input type="hidden" name="estado" value="respondido">
                                                                <button type="submit" class="dropdown-item">
                                                                    <i class="bi bi-check-circle text-success me-2"></i>Respondido
                                                                </button>
                                                            </form>
                                                        </li>
                                                    </ul>
                                                ...
                                                <button class="btn btn-outline-danger" 
                                                        onclick="confirmarEliminacion(<%= contacto.getIdContacto() %>, '<%= contacto.getNombre().replace("'", "\\'") %>', '<%= contacto.getAsunto().replace("'", "\\'") %>')" 
                                                        title="Eliminar">
    ...
    <script>
        ...
        function confirmarEliminacion(idContacto, nombre, asunto) {
            ...
            document.getElementById('deleteForm').action = '<%= request.getContextPath() %>/admin/eliminar-contacto/' + idContacto;
